...
''' PathCompressionQuickUnion Weighted with path compression Union-Find algorithm implementation '''
class PathCompressionQuickUnionUF(object):
	...
	def __root__(self, idx):
		...
		while(idx != self.__id__[idx]):
			...
			self.__id__[idx] = self.__id__[self.__id__[idx]]
			if (self.__pdebug__):
				print("\tNow Root of {} is {}".format(idx, self.__id__[idx]))
			idx = self.__id__[idx]
		return idx
...
if __name__ == '__main__':
	uf = PathCompressionQuickUnionUF(10, True)
	uf.union(0,8)
	uf.union(2,3)
	uf.union(1,3)
	uf.union(5,0)
	...
	print("2:9 => {}".format(uf.connected(2,9))) # False